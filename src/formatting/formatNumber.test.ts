...
describe("formatNumber function", () => {
  test("returns a formatted number without locale or options", () => {
    expect(formatNumber(123456)).toBe("123,456");
  });

  test("returns a formatted number with locale", () => {
    expect(formatNumber(123456, "de-DE")).toBe("123.456");
  });

  test("returns a formatted number with locale and options", () => {
    expect(
